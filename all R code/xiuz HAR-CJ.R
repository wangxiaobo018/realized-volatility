...
#------------------------样本外1200
har_cj_r <- har_cj %>% 
  dplyr::select( BV,JV,day)%>%
  cbind(RV$RV)
colnames(har_cj_r) <- c("BV","JV","DT","RV")


split_index <- floor(0.8*nrow(har_cj_r))
train_data5 <- har_cj_r[(1:split_index),]
test_data5<- har_cj_r[(split_index+1):nrow(har_cj_r),]


harcjo <- data.frame(RV=data_get_cj$RV,BV=har_cj_r$BV,DT=har_cj_r$day)
harcjo <- na.omit(harcjo)
split_index <- floor(0.8*nrow(harcjo))
train_data5 <- harcjo[(1:split_index),]
test_data5 <- harcjo[(split_index+1):nrow(harcjo),]

o_train <- data.frame(RV=train_data5$RV,BV=train_data5$BV,DT=train_data5$DT)
o_test <- data.frame(RV=test_data5$RV,BV=test_data5$BV,DT=test_data5$DT)
har_cj_result <- HARmodel(as.xts(o_train),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
summary(har_cj_result)

predict_o1 <- numeric(nrow(test_data5))
predict_o5 <- numeric(nrow(test_data5))
predict_o22 <- numeric(nrow(test_data5))
for(i in 1:nrow(test_data5)){
  train_start <- i
  train_end <- i + nrow(train_data5) - 1
  train_windows <- harcjo[train_start:train_end,]
  train_windows <- as.xts(train_windows,order.by=as.Date(train_windows$DT))
  model <- HARmodel((train_windows),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
  predict_o1[i] <- predict(model,newdata=harcjo[train_end+1,])
  predict_o5[i] <- predict(model,newdata=harcjo[train_end+5,])
  predict_o22[i] <- predict(model,newdata=harcjo[train_end+22,])
}
o <- data.frame(RV = o_test$RV, RV_pred = predict_o1, RV_pred5 = predict_o5, RV_pred22 = predict_o22)
write.csv(o,"ocj.csv")
...
for(i in 1:nrow(test_data5)){
  ...
  # 预测下一步
  model <- HARmodel(as.xts(train_windows),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
  predict_o1[i] <- predict(model,n=1)
  
  # 递归预测未来5步
  for(j in 1:5){
    train_windows <- rbind(train_windows, predict_o1[i])
    model <- HARmodel(as.xts(train_windows),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
    predict_o5[i] <- predict(model,n=1)
  }
  ...
  for(j in 1:22){
    train_windows <- rbind(train_windows, predict_o1[i])
    model <- HARmodel(as.xts(train_windows),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
    predict_o22[i] <- predict(model,n=1)
  }
}
o <- data.frame(RV = o_test$RV, RV_pred = predict_o1, RV_pred5 = predict_o5, RV_pred22 = predict_o22)
write.csv(o,"ocj.csv")
...
train_df <- har_cj_r[1:(nrow(har_cj_r)-1200), ]
test_df <- har_cj_r[(nrow(har_cj_r)-1199):nrow(har_cj_r), ]
last_600 <- numeric(nrow(test_df))

for(i in 1:nrow(test_df)){
  train_start <- i
  train_end <- i + nrow(train_df) - 1
  train_windows <- har_cj_r[train_start:train_end, ]
  model <- HARmodel(as.xts(train_windows),periods=c(1,5,22),periodsJ=c(1,5,22),type="HARJ")
  last_600[i] <- predict(model,n=1)
}

out_r2t <- data.frame(RV = test_df$RV, RV_pred = last_600) 
write.csv(out_r2t,"cj1200.csv")
